## pPEGpl, a *pPEG* pack for SWI-Prolog
...
For the *pPEG* specification see [*pPEG*][pPEGrepo].

#### Pack `pPEG` for SWI-Prolog

`pPEG` is an add-on pack for SWI-Prolog implemented entirely in Prolog which complements the parsing capabilities of regular expressions (`library(pcre)`) and DCG's already available to the SWI-Prolog community. It consists of a single module (`pPEG`) with three core interface predicates: `peg_compile`, for constructing a parser term from the grammar source (analogous to `pcre:re_compile`), `peg_parse`, for parsing a input string in the language specified by the parser term (analogous to `pcre:re_matchsub`), and `peg_grammar` for accessing the `pPEG` source grammar, largely for documentation/specification purposes. The remainder of this section is an overview of how to use `pPEG` in Prolog applications; for the nitty-gritty details see the [`pPEG` API reference][pPEGref].
...
(Note that a quasi-quoted string cannot contain the substring "`|}`", hence the use of two literals in the `_eos` rule.) A parser term is built from the grammar source using `peg_compile/2`. (More flexible versions of `peg_compile` and `peg_parse` which include an options argument are documented in the [API reference][pPEGref]).

		?- qqs_grammar(QS), peg_compile(QS,qqs).
...
This example doesn't do much that's useful other than provide an example of usage so let's look at a more practical example, a `pPEG` parser for JSON. Without explaining the details, here's the grammar which will be compiled and stored under the name '`json`':
