...
```
When this is parsed by the Prolog system, the result is a grammar term that can then be used to parse text content defined by the grammar (see below). Any arguments to the "Syntax" term, e.g., `pPEG(optimise(true))`, are treated as compile options.
